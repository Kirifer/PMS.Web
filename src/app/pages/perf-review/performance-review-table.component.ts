...
@Component({
  ...
  `,
  imports: [ReactiveFormsModule, LucideAngularModule, HttpClientModule, CommonModule],
...
export class PerformanceReviewTableComponent implements OnInit {
  ...
  onSubmit() {
    if (this.addUserForm.valid) {
      ...
      this.http.post<any>('https://localhost:7012/performance-reviews', performanceData).subscribe({
        next: (response) => {
  ...
  deleteRecord(id: string) {
    ...
    if (isDelete) {
      this.http.delete<any>(`https://localhost:7012/performance-reviews/${id}`).subscribe({
        next: () => {
          // Remove the record from the local array
          this.performanceReviews = this.performanceReviews.filter((record) => record.id !== id);
          this.allPerformanceReviews = this.allPerformanceReviews.filter((record) => record.id !== id);
        },
        error: (err) => {
