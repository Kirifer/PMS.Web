    ...
    >
      <div class="bg-white rounded-lg shadow-lg max-h-[80vh] w-3/4 p-6">
        ...
        <div class="flex justify-between items-center">
          <h2 class="text-lg font-semibold text-gray-800">Edit a Record</h2>
          <button
            (click)="closeDialog()"
...
export class EditHahaPerformanceReviewComponent implements OnInit {
  ...
  fetchPerformanceReview(): void {
    this.http
      .get<any>(`https://localhost:7012/performance-reviews/${this.id}`)
      .subscribe(
        (data) => {
          if (data?.data) {
            console.log(data.data.goals[0].id);
            this.goalId = data.data.goals.map((goal: any) => goal.id);
            this.competencyId = data.data.competencies.map(
              (competency: any) => competency.id
            );

            console.log('Stored goalId:', this.goalId);
            console.log('Stored competencyId:', this.competencyId);
          }
        },
        (error) => console.error('Error fetching Performance Review:', error)
      );
  }
  ...
  submitForm() {
    ...
    const Payload = {
      ...
      goals: this.goalsData.map((goal: any, index: number) => ({
        id: this.goalId[index] || '', // Include goal ID if present
      ...
      competencies: this.competencyData.map((competency: any, index: number) => ({
        id: this.competencyId[index] || '', // Include competency ID if present
    ...
    this.http
      .put<any>(
        `https://localhost:7012/performance-reviews/${this.id}`,
        Payload
      )
      .subscribe(
        (response) => {
        ...
        (error) => {
          ...
          this.showToast('Error updating record. Please try again.');
        }
      );
  }
