...
@Component({
  selector: 'app-update-perform-rev',
  standalone: true,
  imports: [
    FormsModule,
    MatDialogModule,
  ...
  templateUrl: './update-performance-rev.component.html',
  styleUrl: './update-performance-rev.component.css',
})
export class UpdatePerformRevComponent implements OnInit {
  ...
  constructor(
    private dialogRef: MatDialogRef<UpdatePerformRevComponent>,
    ...
    this.employeeObj.competencies = Array.from(
      { length: 5 },
      () => new Competency()
    );
  }
  ...
  submitForm() {
    const payload: Payload = {
      ...
      startDate: `${this.employeeObj.startDate.year}-${String(
        this.employeeObj.startDate.month
      ).padStart(2, '0')}-${String(this.employeeObj.startDate.day).padStart(
        2,
        '0'
      )}`,
      endDate: `${this.employeeObj.endDate.year}-${String(
        this.employeeObj.endDate.month
      ).padStart(2, '0')}-${String(this.employeeObj.endDate.day).padStart(
        2,
        '0'
      )}`,
      ...
      goals: this.employeeObj.goals.map((goal, index) => ({
        ...
        goals: goal.goals,
        weight: goal.weight,
        date: `${this.employeeObj.startDate.year}-${String(
          this.employeeObj.startDate.month
        ).padStart(2, '0')}-${String(this.employeeObj.startDate.day).padStart(
          2,
          '0'
        )} - ${this.employeeObj.endDate.year}-${String(
          this.employeeObj.endDate.month
        ).padStart(2, '0')}-${String(this.employeeObj.endDate.day).padStart(
          2,
          '0'
  ...
  getDescription(rowIndex: number): {
    description: string;
    competencyId: string | null;
  } {
    const selectedCompetency =
      this.employeeObj.competencies[rowIndex].competency;
