...
>
  <div class="p-4 flex justify-between items-center">
    ...
    >
      ...
      />
      <h1 class="font-bold text-xl ml-2 transition-opacity duration-200" *ngIf="isExpanded">
        IT Squarehub
      </h1>
    </div>
  </div>
  ...
  <div class="flex flex-col gap-2 mt-4 flex-grow">
    <a
      *ngFor="let item of navItems"
      ...
      class="nav-item flex items-center gap-2 p-2 hover:bg-blue-950 rounded-lg transition-all ease-in-out duration-300"
      [ngClass]="{
  ...
  <div class="flex flex-col gap-2 mt-auto border-t border-gray-400">
    <a
      *ngFor="let item of userActions"
      [routerLink]="item.link"
      class="nav-item flex items-center gap-2 p-2 cursor-pointer hover:bg-blue-950 rounded-lg transition-all ease-in-out duration-300"
      [ngClass]="{
        'justify-center': !isExpanded,
...
export class SideNavComponent implements OnInit {
  ...
  userActions: NavItem[] = [
    { icon: this.UserIcon, label: 'John Doe', role: 'Administrator', link: '/user-profile' },
    { icon: this.LogOutIcon, label: 'Logout', link: '', action: () => this.logout() },
  ];
